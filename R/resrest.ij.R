...
#'
#' @importFrom stats rest
#' @author Ricardo I. Alcala \email{ricardoi.alcalab@gmail.com}
#' @export
#' @param path Path to the target directory
#' @param prefix Regular expression to manage file names
#' @return A data frame of total leaf spot area for each sample
#' @return \item{sample}{Name of sample}
#' @return \item{total.leafspot.area}{Total leaf spot area of the sample (cm2)}
#' @examples
...
resrest.ij <- function(path, prefix = "\\.|-"){
  ...
  res <- aggregate(temp.data["size"], temp.data["file.name"], "-")
  res$size <- res$size[,1] - res$size[,2]
  names(res) <- c("sample", "total.leafspot.area")
  return(res)
}
