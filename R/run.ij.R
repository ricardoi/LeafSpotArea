...
#' @seealso \code{\link{resmerge.ij}}, \code{\link{resrest.ij}}, \code{\link{readtext.ij}}, \code{\link{eximg}}
#' @examples
...
run.ij <- function(path.imagej = NULL, set.memory = 4, set.directory,
                   ...
                   check.image = F, leaf = "area", save.image = F){
  ...
  # }
  #
# calculate leaf spot or leaf area

  if ( leaf == "spot" ) {
    res <- resrest.ij(path = temp, prefix = prefix)
   } else if (leaf == "area"){
    res <- resmerge.ij(path = temp, prefix = prefix)
  } else {
    print("use leaf = spot or area")
  }
